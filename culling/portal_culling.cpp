...
void add_title(const std::string &text)
{
    ...
    // style = 1 -> plain (default)
    path.set_pos(1.0/a2d.get_sx()-0.1, 0, -1 + 0.09); // BottomRight == (1,0,-1)
    path.set_scale(0.08);
...
void init()
{
    ...
    cellmanager->load_cells_from_model("models/cells");
    // Load portals
    cellmanager->load_portals_from_model("models/portals");
